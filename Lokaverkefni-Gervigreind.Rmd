...
for (i in 1:m) {
  for(k in 1:l){
    boostCol = gbm(loss ~ ., data = data.try.train,
                    ...
                    n.trees = ntree[k], 
                    shrinkage = lambd[i])
    testPred = predict(boostCol, val.small, n.trees = ntree[k])
    testErr[i,k] = mean(abs(exp(val.small$loss) - exp(testPred)))
    print(i)
  }
}
...
boost.pred2 <- predict(boost.fit,
                      data.submit,
                      n.trees = 1000)
MAEBoost = mean(abs(exp(test.small$loss) - exp(boost.pred)))
MAEBoost 
```
