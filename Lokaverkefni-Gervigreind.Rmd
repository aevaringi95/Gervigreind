...
```{r}
length(levels(data$cat116))
length(which(table(data$cat116)==2))
```
...
MAE.try = mean(abs(exp(data.try.test$loss) - exp(pred.try)))
MAE.try
...
```{r}
n3 = dim(data.small.lasso.train)[1]
val = sample(n3,floor(0.2*n3)) 
data.small.lasso.val = data.small.lasso.train[val,]
data.small.lasso.train = data.small.lasso.train[-val,]
```

```{r}
set.seed(1000)
lambd <-  seq(0.05, 0.5, by=0.05)
ntree <- seq(200,2200,by=400)
m <- length(lambd)
l <- length(ntree)
testErr2 <- array(dim=c(m,l))
for (i in 1:m) {
  for(k in 1:l){
    boostCol = gbm(loss ~ ., data = data.small.lasso.train,
                    distribution = "gaussian",
                    n.trees = ntree[k], 
                    shrinkage = lambd[i])
    testPred = predict(boostCol, data.small.lasso.val, n.trees = ntree[k])
    testErr2[i,k] = mean(abs(exp(data.small.lasso.val$loss) - exp(testPred)))
    print(i)
  }
}
testErr2
...
```{r}
set.seed(1000)
boost.fit.small <- gbm(loss ~ ., data = data.small.lasso.train,
                 distribution = "gaussian",
                 n.trees = 5000, shrinkage =best.lambd )
boost.pred.small <- predict(boost.fit.small,
                      data.submit,
                      n.trees = 5000)
MAEBoost.small = mean(abs(exp(data.small.lasso.test$loss) - exp(boost.pred.small)))
MAEBoost.small 
```
...
train_y = data.small.lasso.train$loss
train_x = data.small.lasso.train[,names(data.small.lasso.train)!='loss']

test_y = data.small.lasso.test$loss
test_x = data.small.lasso.test[,names(data.small.lasso.test)!='loss']
...
MAEBoost.small2 = mean(abs(exp(data.small.lasso.val$loss) - exp(y_hat)))
MAEBoost.small2 
